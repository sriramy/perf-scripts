...
## perf.sh --
##
##   Help script for the xcluster ovl/perf.
##
...
test_start() {
	test -n "$__nvm" || export __nvm=2
	test -n "$__nrouters" || export __nrouters=1
	test -n "$timeout" || timeout=10
	test_start_empty $@
	$XCLUSTER tcpdump --start 1 eth1;
	tlog "Capture packets for $timeout seconds";
	otc 1 dump_kallsyms
	rcp 1 /tmp/kallsyms /tmp
	otc 1 "run_perf tx-latency sleep $timeout"
	rcp 1 /tmp/perf.data /tmp
	$XCLUSTER tcpdump --get 1 eth1 >&2
}
